...
4. 筛选正式原题，去重后按网页顺序连续编号；题卡保留原题原文、规范问题、URL、页面标题、小节、岗位/日期、位置和核验日期。
...
- 卡内“原题原文”保持页面原句；另用 `normalized_question`、H1 和文件名保存可独立作答的规范问题。
- 规范问题只允许清理叙述前缀、网页列表符号、多余引号、破折号、回答残片、标点噪声和明显口语病句，不得改变技术含义或增加新考点。
- 指代只有在同一面经上下文中能够唯一还原时才补全；无法可靠还原的残句不入库。
